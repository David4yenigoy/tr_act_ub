...
#             print(coinlist[i], datetime.datetime.now(), "< RSI 3 > :", now_rsi_3)
            ...
            if now_rsi_30 <= 28 :
                r30_lower28[i] = True
            elif now_rsi_30 >= 30 and total < 95000 and r30_lower28[i] == True :
                buy(coinlist[i])
                r30_hold[i] = True
                print(coinlist[i], datetime.datetime.now(), "RSI 3 Sell")
            elif now_rsi_30 >= 60 and cur_price >= profit_price :
                sell(coinlist[i])
                r30_lower28[i] = False
                r30_hold[i] = False 
                print(coinlist[i], datetime.datetime.now(), "RSI 3 Sell")
                ...
                lower28[i] = True
            elif now_rsi_3 >= 30 and total < 95000 and lower28[i] == True and r30_hold[i] == False :
                buy(coinlist[i])
                print(coinlist[i], datetime.datetime.now(), "RSI 3 Buy")
